...
R(x)=Ln;%wavelength in Angstroms
Q(x)=LA;%wavelength in nanometres
S(x)=Lp;%wavelength in nanometres
X(x)=x; % :p
x=x+1;
end
R=R.';
...
Results=table(X,T,Q,R,S,empty_1,empty_2);
...
e_beam_wavelength=Results; %enter table name here
clear R X a amass c e h kev t T m0  tmax x Results L LA Ln Q v Lp S empty_1 empty_2
%% Plot Results
plot(e_beam_wavelength.Accelerating_Voltage_keV, e_beam_wavelength.Wavelength_nm,'Color', 1/255*[255 0 0],'LineWidth',2);
xlabel('Accelerating Voltage / keV','FontSize',36,'FontName','Calibri');
ylabel('Wavelength (non-relativitic) / nm','FontSize',36,'FontName','Calibri');
