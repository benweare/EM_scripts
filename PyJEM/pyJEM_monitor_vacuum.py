...
# This class creates and manages the main dialog
class Main_Dialog:
    # Python constructor - use to create the main dialog 
    def __init__(self, master):
        ...
        self.plot1.set_ylim([50, 100])
        self.plot1.set_xlim([0, 30])
        self.x_data = [0]
        self.y_data = [0]
    ...
    def _update_graph( self ):
        '''
        Update the graph by drawing it with the new data.
        '''
        self.plot1.clear()
        self.plot1.plot( self.x_data, self.y_data )
        self.plot1.set_ylim([50, 100])
    ...
    # Python Destructor - called when the main dialog is closed. Could be used for stopping a thread
    # closing a file, restoring default settings etc. Destructors are optional    
    def __del__(self):
        #print("Destructor called when Main Dialog is closed.")
    ...
    def monitor_emission(self):
        ...
        # infinite sampling loop Broken out of by pressing Stop
        
        while 1:
            # Source the emission
        
            my_emission=self.vac_module.GetPegInfo( 0 )[0]
            ...
            print("\rPenning 0 = "+str("%.2f" % my_emission)+" Time = "+nowtime, end="")
            
            # Update the dialog
            self.EmissionVal.set(str("%.2f" % my_emission))
            ...
            self._update_vals( interval, my_emission )
            self._update_graph()
    ...
    def start_monitoring_response(self):
        ...
        threading.Timer(0, self.monitor_emission).start()
